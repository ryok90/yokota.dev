...
- Only the first pass carries text. `index.astro` emits `--ticker-item-1` through `--ticker-item-8` once on `.ticker`, and `global.css` labels every later span through `span:empty:nth-child(16n + N)::before`, so the crawled document states each topic once instead of twelve times; CSS generated content is not page text. Each item renders a span plus a diamond, which is why the spans of one pass land on odd child positions and repeat every 16 children. That stride and the eight `content` rules are both tied to `tickerItems.length`; changing the list means changing those rules. The block stays `aria-hidden`, so assistive technology never saw the repetition and its behavior is unchanged.
...
- `public/images/profile-s.webp` is the 256x256 portrait referenced by Rodrigo's email signature, cropped from `src/assets/portrait-square.jpg`. It is a hotlinked external dependency, not used by any page, so the path and the square aspect must stay stable; regenerate it from the same source if the portrait changes.
